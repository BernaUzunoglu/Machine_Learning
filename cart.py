...
#  bazı uyarıları görmemek için ignore ediyoruz
...
# Bağımlı değişkeni seçelim
y = df["Outcome"]
# Bağımısz değişkenleri seçelim
...
# ROC eğirisi için 1. sınıfa ait olma olasılığı
# AUC için y_prob:
...
# Sonuçlara göre 1 modelim çok doğru çalışıyor gibi burda bir düşünmek gerek modelin rassal mı olması durumu, bunun için başarı değerlendirme işlemerine bir baklaım.
#####################
...
# roc değeri : 0.65584 anlaşılacağı üzere model train setinde 1 olacak şekilde değer oluştururken görmediği test verilerinde nerdeyse %50 değer düşüşü oluştu. Burdan train setinde veriyi aşırı öğrenmiştir(overfitting) yorum yapabiliriz.


#####################
# CV - Cross Validation Çapraz Doğrulama  ile Başarı Değerlendirme
#####################
...
# Model başarımını nasıl artırabiliriz?
# Yeni gözlemler ekleyerek. Feature engineering ve veri işleme kısımlarına dokunarak ya da hiper parametrelerini ayarlayarak.
...
# Grid Search Cv ile verilen parametrelere göre bir arama yapalım.
# Grid Search içerisinde en iyi modeli saklar.
...
# En iyi hiperparametre değerlerine bakalım
cart_best_grid.best_params_

# En iyi değerlere karşı en iyi score verir bu fonk. score parametresinin ön tanımlı değeri accuracydir.
...
# 5. Final Model
################################################
# En iyi parametreler ile modeli final modeli kuralım.
cart_final = DecisionTreeClassifier(**cart_best_grid.best_params_, random_state=17).fit(X, y)
cart_final.get_params()

# En iyi parametreleri set etip fit edelim.
...
# Değişkenlerin önem düzeylerini getirelim
cart_final.feature_importances_

def plot_importance(model, features, num=len(X), save=False):
    # Feature importances değerleri ile featureları listede tutuyoruz.
    feature_imp = pd.DataFrame({'Value': model.feature_importances_, 'Feature': features.columns})
    plt.figure(figsize=(10, 10))
    sns.set(font_scale=1)
    # Feature importaces değerlerine göre küçükten büyüğe sıralıyoruz. Barpşot ile görselleştiriyoruz.
...
################################################
# 7. Analyzing Model Complexity (Model Karmaşıklığı) with Learning Curves (BONUS)
################################################
# verilen parametrenin değerine göre model her bir denenme değerinin gösterilmesi
...
# 10 capraz doğrulama 10 tane değer
...
# Ağacın dallanmış halinin görsel halini çizelim
...
# Karar Kurallarının Python - SQL - Excel kodlarını Çıkarma
################################################
